...
export default {
  name: 'AdminDetailedQuestionEdit',
  props: {
    question: {
      type: Object,
      required: true
  ...
  data() {
    return {
      valid: false,
      localQuestion: { ...this.question }, // 로컬 데이터로 복사
      contentRules: [
        v => !!v || '질문 내용을 입력하세요'
      ]
    };
  ...
  methods: {
    ...mapActions('detailedquestion', ['updateDetailedQuestion']),
    ...mapActions('deficiency', ['fetchDeficiencies']),
    async save() {
      if (this.$refs.form.validate()) {
        try {
          await this.updateDetailedQuestion(this.localQuestion);
          this.$emit('save');
        } catch (error) {
  ...
  async created() {
    console.log('AdminDetailedQuestionEdit created.');
