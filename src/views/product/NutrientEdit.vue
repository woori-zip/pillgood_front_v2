        ...
        <tbody>
          <tr v-for="ne in transformedData" :key="ne.id">
            ...
            <td>
              ...
              <button @click="handleRemoveNutrientEfficiency(ne.nutrientEfficiencyId)">삭제</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...mapActions('nutrient', ['fetchNutrientEfficiencies', 'fetchNutrients', 'removeNutrientEfficiency']),
    ...
    async handleRemoveNutrientEfficiency(id) {
      try {
        await this.removeNutrientEfficiency(id);
        await this.loadData(); // 삭제 후 데이터 다시 로드
      } catch (error) {
